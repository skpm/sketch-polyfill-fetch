  ...
  return new Promise(function (resolve, reject) {
    ...
    if (options.body) {
      ...
      if (typeof options.body === 'string') {
        ...
        data = options.body
      } else if (option.body._isFormData) {
        data = option.body._data
        data.appendData(
          NSString.alloc()
            .initWithString("--" + boundary + "--\r\n")
            .dataUsingEncoding(NSUTF8StringEncoding)
        )
        request.setValue_forHTTPHeaderField('multipart/form-data; boundary=' + option.body._boundary, 'Content-Type')
      } else {
        var error
